...
--
-- price.tb_<code> 저장용
-- select insert_price('public','rm_test' ,ARRAY[ARRAY[1,2,3] ,ARRAY[10,20,30] ,ARRAY[100,200,300]]);
---------------------------------
CREATE
OR REPLACE FUNCTION insert_price(schema_nm text,tb_nm text, arr numeric [ ] [ ]) RETURNS INTEGER AS $$
DECLARE
    res INTEGER;
    q text;
    i integer;
...
BEGIN
    ...
    q := format(' INSERT INTO  %I.%I ("p_date", "op", "hp", "lp", "cp", "vol", "fb_rate")  VALUES ',schema_nm,tb_nm);
...
--
-- price.tb_<code> 저장용
-- select insert_or_update_price('public','rm_test' ,ARRAY[ARRAY[1,21,31] ,ARRAY[10,210,310] ,ARRAY[100,2100,3100]]);
---------------------------------
CREATE
OR REPLACE FUNCTION insert_or_update_price(schema_nm text,tb_nm text, arr numeric [ ] [ ]) RETURNS INTEGER AS $$
DECLARE
    res INTEGER;
    q text;
    i integer;
